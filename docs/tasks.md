...
- [x] FR-21: Catalogue integrity check on startup (unique IDs, valid refs, no circular deps)
- [x] FR-19: Kit persistence — save kit to `data/kits/<id>.json`
- [x] FR-19: Auto-save with debounce (~500ms); immediate save on close/switch/report
- [x] FR-20: Kit loader — read all `data/kits/*.json` on launch; skip missing catalogue items with warning
- [x] Wire up screen routing (`AppController`) with navigation between Kit List and Build screens
...
- [x] Update `styles.py`: add `CATALOGUE_WIDTH`, `KIT_PANEL_WIDTH`, `TILE_SIZE`, `ICON_SIZE`
...
- [x] FR-06: Warning indicator (⚠) on category cards for uncovered categories
- [x] FR-06: Click category → item grid view (2-column cards with icon + label)
- [x] FR-06: Back button returns to category grid
- [x] FR-07: Item tooltip showing weight, calories, water, notes
...
- [x] FR-11: Packed items grouped by category with collapsible headers; compact −/+ qty controls

---

## Kit Management

- [x] FR-01b: Kit List screen — 3-column card grid with name, readiness score, weight, last modified
- [x] FR-01b: Empty state when no kits exist
- [ ] FR-01: "+ New Kit" button — opens Kit Configuration modal, then navigates to build screen
- [x] FR-02: Inline rename on build screen header (click → text field → Enter/blur saves)
- [x] FR-03: Copy kit from Kit List card ("Copy of [name]", new unique ID)
- [x] FR-04: Delete kit from Kit List card (removes JSON file from disk)
...
- [x] FR-13: Weight bar — fill proportion vs limit; green <75%, amber 75–99%, red ≥100%
...
- [x] FR-14: Calories and water shown as fraction of requirement (people × duration)
- [x] FR-14b: Warning indicator on category cards for uncovered categories (live)
